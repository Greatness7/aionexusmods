...
USER_AGENT = "{}/{} ({}; {}) {}/{}".format(
    __package__,
    "0.1.4",  # importlib.metadata.version(__package__)
    platform.platform(),
...
class NexusMods:
    ...
    async def get_updated(self, period: str) -> list[ModUpdate]:
        ...
        result = await self._get(f"{BASE_URL}/games/{self.game_domain_name}/mods/updated.json", data=data)
    ...
    async def get_changelogs(self, mod_id: int) -> dict[str, list[str]]:
        ...
        """
        result = await self._get(f"{BASE_URL}/games/{self.game_domain_name}/mods/{mod_id}/changelogs.json")
    ...
    async def set_tracked(self, mod_id: int, tracked: bool) -> Message:
        ...
        if tracked:
            result = await self._post(f"{BASE_URL}/user/tracked_mods.json", data=data)
        else:
            result = await self._delete(f"{BASE_URL}/user/tracked_mods.json", data=data)
